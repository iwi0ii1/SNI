...
[[nodiscard]]
const struct k64_hwd_firmware_acpi_rsdp_descriptor_t* const k64_hwd_firmware_acpi_find_rsdp(const uint32_t start, const uint32_t end) {
    for (uintptr_t addr = start; addr < end; addr += 16) {
        ...
        const struct k64_hwd_firmware_acpi_rsdp_descriptor_t* const rsdp = (struct k64_hwd_firmware_acpi_rsdp_descriptor_t*)p;

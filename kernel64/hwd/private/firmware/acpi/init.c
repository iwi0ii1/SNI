...
void k64_hwd_firmware_acpi_init(void) {
    ...
    #ifdef DEBUG_FORM
    if (!rsdp) {
        shared_text_aputs("rsdp is being NULL, again!!", 0x0F);
        while (1)
            __asm__("hlt");
    }
    #endif
    ...
    if (rsdp->revision == 0 && rsdp->rsdt_address) {
        shared_text_aputs("Inside ACPI v1. ", 0x0F);
        ...
        shared_text_aputs("Inside ACPI v2+. ", 0x0F);
        ...
        #ifdef DEBUG_FORM
        shared_text_aputs("Neither inside ACPI v1 nor v2+... k64_hwd_firmware_acpi_find_rsdp(0x0E0000, 0x100000) could've been wrong.", 0x0F);
        #endif
        while (1)
            __asm__("hlt");
    }
    ...
    if (shared_mem_cmp(k64_hwd_firmware_acpi_table.madt->header.signature, "APIC", 4) != 0) {
        shared_text_aputs("k64_hwd_firmware_acpi_table.madt->signature isn't \"APIC\"...", 0x0F);
        while (1)
            __asm__("hlt");
    }
    #endif

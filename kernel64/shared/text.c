...
static void shared_text_internal_scroll(const uint8_t rep) {
    if (rep == 0)
        return;
...
void shared_text_putc(const unsigned char ch, const uint8_t attr) {
    if (cursor_pos > VGA_LAST_SLOT) // cursor_pos must be at least 2000 to reach this
        shared_text_internal_scroll(1);
...
void shared_text_aputs(const unsigned char* const ch_ptr, const uint8_t attr) {
    for (uint16_t i = 0; ch_ptr[i] != '\0'; ++i)
        shared_text_putc(ch_ptr[i], attr);
}



void shared_text_fill_char(const unsigned char ch, const uint8_t attr) {
    cursor_pos = 0;
    for (uint16_t i = 0; i <= VGA_LAST_SLOT; ++i)
        shared_text_putc(ch, attr);
...
void shared_text_reposition_cursor(const uint8_t x_coord, const uint8_t y_coord) {
    cursor_pos = (y_coord << 6) + (y_coord << 4) + x_coord;
}



uint16_t shared_text_get_cursor_pos(void) {
...
void shared_text_newline_cursor(const uint8_t rep) {
    if (rep == 0)
        return;

    const uint16_t pos = shared_text_get_cursor_pos();
    const uint8_t x = (const uint8_t)(pos >> 8);
    const uint8_t y = (const uint8_t)(pos & 0xFF); // AND there to preserve bits

    if (y + rep >= VGA_HEIGHT)
        shared_text_internal_scroll(rep);

    shared_text_reposition_cursor((uint8_t)0, y + rep);
}
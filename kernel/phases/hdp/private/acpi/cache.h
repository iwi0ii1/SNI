...
[[nodiscard]]
static inline struct hdp_acpi_table_t hdp_acpi_cache_tables_rsdt(const struct hdp_acpi_rsdt_t* const rsdt) {    
    #ifdef DEBUG_FORM
    if (!rsdt) {
    ...
    for (uint32_t i = 0; i < entries; i++) {
        const struct hdp_acpi_sdt_header_t* const hdr = (const struct hdp_acpi_sdt_header_t*)(uintptr_t)ptrs[i];

        if (!shared_mem_cmp(hdr->signature, "MCFG", 4))
            table.mcfg = (struct hdp_acpi_mcfg_t*)(uintptr_t)ptrs[i];
        else if (!shared_mem_cmp(hdr->signature, "APIC", 4))
            table.madt = (struct hdp_acpi_madt_t*)(uintptr_t)ptrs[i];
        else if (!shared_mem_cmp(hdr->signature, "FACP", 4))
            table.fadt = (struct hdp_acpi_fadt_t*)(uintptr_t)ptrs[i];
        else if (!shared_mem_cmp(hdr->signature, "HPET", 4))
            table.hpet = (struct hdp_acpi_hpet_t*)(uintptr_t)ptrs[i];
    }
...
[[nodiscard]]
static inline struct hdp_acpi_table_t hdp_acpi_cache_tables_xsdt(const struct hdp_acpi_xsdt_t* const xsdt) {
    #ifdef DEBUG_FORM
    if (!xsdt) {
    ...
    for (uint32_t i = 0; i < entries; i++) {
        ...
        if (!shared_mem_cmp(hdr->signature, "MCFG", 4))
            table.mcfg = (struct hdp_acpi_mcfg_t*)ptrs[i];
        else if (!shared_mem_cmp(hdr->signature, "APIC", 4))
            table.madt = (struct hdp_acpi_madt_t*)ptrs[i];
        else if (!shared_mem_cmp(hdr->signature, "FACP", 4))
            table.fadt = (struct hdp_acpi_fadt_t*)ptrs[i];
        else if (!shared_mem_cmp(hdr->signature, "HPET", 4))
            table.hpet = (struct hdp_acpi_hpet_t*)ptrs[i];
    }
